...
- **Mesh traffic (Prometheus)**: Istio / Linkerd / Cilium metrikleri için aynı origin `/prometheus` kullanılır. Kümede hedef, Deployment env **`PROMETHEUS_UPSTREAM`** (`host:port`, örn. `prometheus.azure-extensions-usage-system.svc.cluster.local:9090`) ile verilir; `k8s-manifest.yaml` içinde güncellenir. Yerelde Vite `vite.config.js` ile `/prometheus` → `127.0.0.1:9090` (port-forward).
